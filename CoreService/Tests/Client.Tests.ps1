...
Describe "Core Service Client Tests" {
	...
	Context "Get-TridionCoreServiceClient" {
		InModuleScope Client {
			...
			# Mocks
			# ***********************
			Mock _Set-Credential {}
			Mock _Set-ImpersonateUser {}
			Mock _New-AssemblyInstance {
			...
			It "does not impersonate and connects as the current user by default" {
				...
				Assert-MockCalled _New-AssemblyInstance -Times 1 -Scope It;
				Assert-MockCalled _Set-ImpersonateUser -Times 0 -Scope It;
				Assert-MockCalled _Set-Credential -Times 0 -Scope It;
			}
			
			It "supports impersonation" {
				$client = Get-TridionCoreServiceClient -ImpersonateUserName 'TEST\User01';
				Assert-MockCalled _New-AssemblyInstance -Times 1 -Scope It;
				Assert-MockCalled _Set-ImpersonateUser -Times 1 -Scope It;
				Assert-MockCalled _Set-Credential -Times 0 -Scope It;
			}
			...
			It "supports connecting as a different user" {
				...
				$client = Get-TridionCoreServiceClient;
				Assert-MockCalled _New-AssemblyInstance -Times 1 -Scope It;
				Assert-MockCalled _Set-Credential -Times 1 -Scope It;
				Assert-MockCalled _Set-ImpersonateUser -Times 0 -Scope It;
			}
		}
	}	
	
	Context "_Get-CoreServiceBinding" {
